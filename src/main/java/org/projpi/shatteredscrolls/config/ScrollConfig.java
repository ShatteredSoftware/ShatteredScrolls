...
public class ScrollConfig implements Cloneable, ConfigurationSerializable
{
    ...
    public ScrollConfig(String unboundName, String unboundLore, String boundName, String boundLocationLore,
                        String boundPositionLore, boolean unboundGlow, boolean boundGlow, int customModelData,
                        Material material, int cooldown, int charges, ScrollCost cost)
    {
    ...
    public static ScrollConfig deserialize(Map<String, Object> map)
    {
        ...
        if (map.containsKey("scroll-unbound-name") && map.get("scroll-unbound-name") instanceof String
                && !((String) map.get("scroll-unbound-name")).isEmpty())
        {
            unboundName = ChatColor.translateAlternateColorCodes('&', (String) map.get("scroll-unbound-name"));
        }
        ...
        {
            unboundLore = ChatColor.translateAlternateColorCodes('&', (String) map.get("scroll-unbound-lore"));
        }
        ...
        {
            boundName = ChatColor.translateAlternateColorCodes('&', (String) map.get("scroll-bound-name"));
        }

        String boundPositionLore = "§7A bound teleportation scroll." +
        ...
        ScrollCost cost = getIfValid(map, "cost", ScrollCost.class,
                new ScrollCost(ScrollCost.CostType.POTION, new PotionEffect(PotionEffectType.BLINDNESS, 5, 0)));

        return new ScrollConfig(unboundName, unboundLore, boundName, boundLocationLore, boundPositionLore,
                unboundGlow, boundGlow, customModelData, material, cooldown, charges, cost);
    }
