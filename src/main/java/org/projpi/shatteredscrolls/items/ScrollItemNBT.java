...
public class ScrollItemNBT
{
    ...
    public static BindingType getBindingType(ItemStack stack)
    {
        if(isBound(stack))
        {
            return nbtUtil.hasString(stack, "shatteredscrolls_destination") ? BindingType.LOCATION : BindingType.POSITION;
        }
    ...
    public static ItemStack bindTo(ItemStack stack, Location location)
    {
        ...
        {
            stack = nbtUtil.setString(stack, "shatteredscrolls_dest_world", location.getWorld().getUID().toString());
        }
